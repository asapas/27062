...
class ListingService extends WooCommerceService {
  ...
  Future<List<Category>> getCategoriesByPage(
    ...
    try {
      ...
      var endpoint =
          '$domain/wp-json/wp/v2/${DataMapping().kCategoryPath}?hide_empty=${kAdvanceConfig.hideEmptyCategories}&_embed&per_page=$limit&page=$page&exclude=$kExcludedCategory';
      if ((lang?.isNotEmpty ?? false) && kAdvanceConfig.isMultiLanguages) {
        endpoint += '&lang=$lang';
      }
      var response = await httpGet(endpoint.toUri()!);
  ...
  Future createReview(ReviewPayload payload) async {
    try {
      if (ServerConfig().isListProType) {
        await httpPost(
          '$domain/wp-json/wp/v2/submitReview'.toUri()!,
      ...
      if (ServerConfig().isListeoType) {
        ...
        final data = payload.toListeoJson();
      ...
      if (ServerConfig().isMyListingType) {
        var request = http.MultipartRequest(
            'POST', Uri.parse('$domain/wp-comments-post.php'));
        final data = payload.toListingJson();
        request.fields.addAll(data);
        await request.send();
      }
    } catch (e) {
  ...
  Future<List<Store>>? getNearbyStores(Prediction prediction,
    ...
    return _multiVendorServices?.getNearbyStores(prediction,
        page: page, perPage: perPage, radius: radius, name: name);
  }
  ...
  Future<PagingResponse<Order>> getMyOrders({
    ...
    if (_multiVendorServices != null) {
      try {
        final response = await httpGet(
          '$domain/wp-json/wp/v2/dokan/orders?page=$cursor'.toUri()!,
          headers: {
